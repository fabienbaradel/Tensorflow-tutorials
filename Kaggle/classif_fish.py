...
class Fish_classif(object):
    ...
    def inference(self):
        ...
        # Set model weights
        self.W = tf.Variable(tf.zeros([2048, 8]))
        self.b = tf.Variable(tf.zeros([8]))

        # Construct the inference
        self.logits = tf.add(tf.matmul(self.X, self.W), self.b) # WX+b : the logits
    ...
    def losses(self):
        ...
        """
        # cross entropy loss
        self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(self.logits, self.Y_one_hot))
