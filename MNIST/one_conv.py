...
class MNIST_logistic(object):
    ...
    def inference(self):
        ...
        # Convolutional layer
        nb_filter = 1
        self.W_conv1 = init_weights([5, 5, 1, nb_filter])
        self.b_conv1 = init_weights([nb_filter])
        net = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(X, self.W_conv1,
        ...
        # Classifier
        net = tf.reshape(net, [-1, 14*14*nb_filter])
        self.W = init_weights([14*14*nb_filter, 10])
        self.b = init_weights([10])
        self.logits = tf.add(tf.matmul(net, self.W), self.b) # WX+b : the logits
