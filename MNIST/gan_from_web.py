...
D_real, D_logit_real = discriminator(X)#dc_discriminator(X)#
D_fake, D_logit_fake = discriminator(G_sample)#dc_discriminator(G_sample)#
...
for it in range(1000000):
    if it % 100 == 0:
        ...
        fig = plot(samples)
        plt.savefig('./img_gan//{}.png'.format(str(i).zfill(3)), bbox_inches='tight')
        i += 1
        plt.close(fig)
