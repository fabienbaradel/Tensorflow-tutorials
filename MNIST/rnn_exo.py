...
class MNIST_logistic(object):
    ...
    def inference(self):
        ...
        # Set model weights
        self.W = ???? # variable of size [100, 10]
        self.b = ??? # biases for a classif of 10 classes

        # Classif using rnn inner loop last output
        self.logits = ?????? # WX+b : the logits using x as the last hidden state
        self.Y_hat = ?????? # softmax
