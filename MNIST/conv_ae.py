...
class Autoencoder(object):
    ...
    def inference(self):
        ...
        def encoder(x):
            #x = tf.random_normal([784])
            X = tf.reshape(x, [-1, 28, 28, 1])
            net = slim.conv2d(X, 10, [3,3], stride=2, padding="SAME")
            net = slim.conv2d(net, 10, [3,3], stride=2, padding="SAME")
            net = slim.conv2d(net, 10, [3,3], stride=2, padding="SAME")
            return net

        # Building the decoder
        def decoder(x):
            #x = tf.random_normal([1, 4, 4, 10])
            net = slim.conv2d_transpose(x, 10, [3,3], stride = 2, padding = "SAME")
            net = slim.conv2d_transpose(net, 10, [3,3], stride = 2, padding = "SAME")
            net = slim.conv2d_transpose(net, 1, [3,3], stride = 2, padding = "SAME")
