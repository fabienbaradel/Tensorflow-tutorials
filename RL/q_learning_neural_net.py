...
Q_values = tf.matmul(input_state, W)
...
diff_tf = Q_target_values - Q_values
loss = tf.reduce_sum(tf.square(Q_target_values - Q_values))
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        while j < 99 and s not in [5,7,11,12,15]:
            ...
            a, all_Q = sess.run([action, Q_values], feed_dict={input_state: s_one_hot})
            ...
            all_next_Q = sess.run(Q_values, feed_dict={input_state: new_s_one_hot})
            ...
            sess.run(train_op, feed_dict={input_state: s_one_hot,
                                          Q_target_values: all_Q_target})
